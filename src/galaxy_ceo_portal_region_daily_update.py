...
def read_region_snapshot(
    ...
    for offset in range(2, max(row_count, 2) + 1, batch_size):
        ...
        values = client.read_range(spreadsheet_token, f"{sheet['sheet_id']}!A{offset}:E{last_row}")
...
def validate_title(client: FeishuClient, spreadsheet_token: str) -> None:
...
def update_once(
    ...
    validate_title(client, spreadsheet_token)
    ...
    if dry_run:
        result.update({"ready": True, "source_latest_date": source_latest.isoformat(), "dry_run": True})
        return result
    ...
    result.update({"ready": True, "source_latest_date": source_latest.isoformat(), "title": new_title})
    return result
...
def run_with_retries(args: argparse.Namespace) -> int:
    ...
    raise RegionUpdateError(f"region source did not reach yesterday; last_result={last_result}")
